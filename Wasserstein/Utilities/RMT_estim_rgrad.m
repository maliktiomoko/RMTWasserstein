...
switch distance
    ...
    case 'Wasserstein'
        ...
        for i=1:length(lambda)
            ...
            all=0;all2=0;
            integrand_real=@(z) (1/(pi*p)).*(1./sqrt((m(z))))./((lambda(i)-z).^2);
            other2=@(z) -(p/c2)*((1-c2)./(z.^2));
            for l=1:length(lambdac)
                other2=@(z) other2(z)-(lambdac(l)-z).^(-2);
            end
            integrand2_real=@(z) (1./(pi*p))*sqrt((1./m(z))).*other2(z);
            for j=1:length(zeta)
                if i==j
                    all=all+integral(@(z) integrand2_real(z),zeta(j),lambda(j));
                else
                    all2=all2+integral(@(z) integrand_real(z),zeta(j),lambda(j));
                end
            end
            out_prim(i,i)=-(all+all2)*lambda(i);
        end
        [diago,indi]=sort(diag(V)); diag_out=diag(out_prim);
        vec=zeros(p,1);
        for i=1:length(indi)
            pos=find(indi==i);
            vec(i)=diag_out(pos);
        end
            my_out=-2*(K*diag(vec)/K)*S+(1/p)*S^(2);
            out_1=real((my_out+transpose(my_out))/2);
            [a,r]=RMT_estim(S,Cs,n2,'Wasserstein');
            out_=2*r*sqrt(RMT_estim(S,Cs,n2,'Wasserstein'))*out_1;
            %out_=out_1;
