...
options  = struct('verbosity',2,'statsfun',@statsfun,'maxiter',100);

%Steepest descent:We can try other(conjuguate gradient,pso)
[C_est,cost,info,opt_out] = steepestdescent(problem,C0,options);
...
elseif strcmp(distance,'Wasserstein')
    metric=@(D) (1/p)*trace(D)+(1/p)*trace(C)-2*trace((D^(1/2)*C*D^(1/2))^(1/2))/p;
else
