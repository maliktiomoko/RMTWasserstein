...
%n_vec=floor(linspace(130,130,1));
n_vec=floor(linspace(100,300,10));
%n_vec=128;
p=90;
...
% Which target covariance (manopt_rand,Wishart,dirac,toeplitz,manual)
    covariance='dirac';
    param.toeplitz=0.4;param.dirac=[0.10;1;3;4];param.wishart=2;
...
for b=1:length(n_vec)
        ...
        case 'Wishart'
            %Either Wishart matrix
             ...
             for k=1:param.wishart*p
                 rd(:,k) = mvnrnd(zeros(1,p),toeplitz(0.8.^(0:p-1)));
             end
    ...
    metric_wass=@(D) (1/p)*trace(D)+(1/p)*trace(C)-2*trace((D^(1/2)*C*D^(1/2))^(1/2))/p;
    % QUEST method
   ...
   switch initialisation
       case 'shrinkage'
            ...
            C0=diag(x0);
            if strcmp(distance,'Fisher') || strcmp(distance,'log') || strcmp(distance,'log1st') || strcmp(distance,'t') || ...
            strcmp(distance,'Battacharrya') || strcmp(distance,'KL') || strcmp(distance,'Wasserstein')
                C0=C0;
            else
                C0=C0^(-1);
            end
    ...
    elseif strcmp(distance,'Wasserstein')
        distance_real_vec(fg)=metric_wass(x*x'/n);
   ...
   elseif strcmp(distance,'Wasserstein')
        distance_ledoi_vec(fg)=metric_wass(sigmahat);
        distance_ledoi2_vec(fg)=metric_wass(sigmahat2);
   else
   ...
   if strcmp(distance,'Wasserstein')
       distance_our_vec(fg)=metric_wass(C_est);
   else
...
legend('SCM\_theo','SCM\_calc','our','Ledoit\_Peche1','Ledoit\_Peche2')
